...
Injection-Schwachstellen tauchen auf, wenn eine Anwendung nicht vertrauenswürdige Daten an einen Interpreter weiterleitet. Injection-Schwachstellen sind weit verbreitet; wir haben schon SQL-Injection und Cross-Site Scripting (XSS) kennengelernt.
...
die als Input vom User bzw. von der Userin kommen, werden mit `execute` an die Datenbank
übergeben. In diesem zweiten Schritt können sie aber nur noch als
Daten, nicht mehr als SQL interpretiert werden.

## Prepared Statement mit benannten Platzhaltern

Eine zweite Schreibweise für Prepared Statements ist noch besser lesbar: dabei
...
Man kann auch mit Prepared Statements noch Code schreiben, der für Injections anfällig ist.
Wenn man nämlich im String der an prepare übergeben wird Variablen einbettet:

<php caption="Prepared Statements falsch gemacht">
$stm = $dbh->prepare ( "UPDATE users SET newsletter = ? WHERE USERNAME = '$name'" );
$stm->execute(array( $_GET['newsletter'] ) );
</php>
...
3. Den Input von der Userin/dem User geeignet escapen, bevor er an den Interpreter weitergegeben wird

Im dritten und schlechtesten Fall ist weiter zu beachten:

- Bei der Validierung des Inputs immer eine ‚White List‘ verwenden, also nur Zeichen erlauben, die harmlos sind (auf der „weißen Liste“ stehen), alle anderen verwerfen

Unabhängig von den oben genannten Punkten gilt noch die Empfehlung:

- Der Web-Applikation nur minimale Zugriffsrechte zur Datenbank gewähren. Damit kann ein eventueller Schaden durch gelungene Injection minimiert werden.


## Weitere Quellen zu SQL Injection

* Wir haben das Problem zuerst besprochen im Kapitel [PHP DB Schreiben: Löschen](https://web-development.github.io/php-db-schreiben/daten-loeschen/)
...
Cross-Site Scripting kann komplett vermieden werden, wenn man niemals Input von Benutzerinnen und Benutzern auf der Webseite wiedergibt.
Das ist ein seltener, aber sehr sicherer Fall.

XSS vermeiden kann man mit zwei Verteidigungs-Linien:

- Schon bei der Eingabe wird alles entfernt, was nicht auf einer „Liste der Erlaubten“ steht
...
also ganz leicht möglich, JavaScript in einem `onmouseover`-Attribut einzuschmuggeln.

Stattdessen sollte man einen HTML-Filter wie [HTML Purifier](http://htmlpurifier.org/) verwenden.
...
META-Tag in HTML an den Client übermittelt. Aktuelle Browser halten die Policy ein.
...
- explizit: das Laden von weiteren Ressourcen für diese Webseite (Bilder, Fonts, JavaScript) erfolgt nur über HTTPS
- implizit: script-Tags im HTML und der JavaScript-Befehl `eval` sind deaktiviert

Falls man `script`-Tags im HTML zulassen will, kann man die Option `'unsafe-inline'` verwenden:
...
- Wir haben das Problem zuerst besprochen im Kapitel [PHP DB Schreiben: Daten Bearbeiten](https://web-development.github.io/php-db-schreiben/daten-editieren/)
